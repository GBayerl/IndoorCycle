/**
 * @version		$Id: map.js 780 2012-05-03 02:21:55Z jeffchannell $
 * @package		JCalPro
...
var jcl_mod_add_map = function(id, lat, lng) {
	...
	if (map.hasMap) return map.element;
	map.lat = lat;
	map.lng = lng;
...
var jcl_mod_refresh_locations = function(id) {
	...
	if (!map.hasMarkers || map.checking) return;
...
var jcl_mod_fetch_locations = function(id, lat, lng) {
	...
	,	onSuccess: function(responseJSON, responseText) {
			...
			Array.each(responseJSON, function(el) {
				...
				if (-1 == zoom) bounds.extend(latlng);
			});
			if (-1 == zoom && changeBounds) {
				map.map.fitBounds(bounds);
				if (16 < map.map.getZoom()) map.map.setZoom(16);
			}
			setTimeout(function() {
				if (!map.hasMarkers) {
