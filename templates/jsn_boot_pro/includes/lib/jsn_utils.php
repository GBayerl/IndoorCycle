 ...
 */
	...
	class JSNUtils {
		...
		{
			$templateName 	= explode( DIRECTORY_SEPARATOR, str_replace( array( '\includes\lib', '/includes/lib' ), '', dirname(__FILE__) ) );
		...
		// 	/* Remove all "jsn_setxxx" parts from the URL */
		// 	// $res = preg_match_all("/jsn_set[a-z]{0,}/", $uri->toString(), $matches);
		// 	// if ($res !== false && $res > 0)
		// 	// {
		...
		// 	/* Set the variable string for the URL */
		...
		public function getTemplateManifestCache()
		{
			...
			$query->select('manifest_cache')
				->from('#__extensions')
				->where("type = 'template'")
				->where("element = '" . $template_defacto_name . "'");
