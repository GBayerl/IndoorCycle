...
class JSNAjax
{
	...
	function installSampleData()
	{
		...
		{
			$obj_sample_data_helper->deleteSampleDataFile($file_name);
			echo json_encode(array('install' => false, 'message' => JText::_('JSN_SAMPLE_DATA_UNABLE_EXTRACT_PACKAGE'), 'redirect_link'=>$link, 'manual'=>true));
			exit();
		}
	...
	function _hasModifiedFiles ()
	{
		require_once($this->_template_folder_path . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'lib' . DIRECTORY_SEPARATOR . 'jsn_checksum_integrity_comparison.php');
		$checksum 	= new JSNChecksumIntegrityComparison();
		$result 	= $checksum->compareIntegrity();

		return count(@$result['modified']) > 0;
	}
	...
	function autoUpdateTemplate()
	{
		...
		if ($this->_hasModifiedFiles()) {
			echo json_encode(array(
				'download'		=> false,
				'file_name'		=> '',
				'connection'	=> true,
				'message'		=> JText::_('JSN_AUTO_UPDATE_ERROR_FOUND_MODIFIED_FILES'),
				'manual'		=> false,
				'redirect'		=> true
			));
			jexit();
		...
		$unpack 					= JInstallerHelper::unpack($package_path);

		if (!empty($unpack))
		{
			try {
				$installer = JInstaller::getInstance();
				$installer->install($unpack['dir']);

				$templatePath = JPATH_ROOT . '/templates/' . $this->_template_name;

				foreach (array('admin', 'elements', 'includes') as $name)
					if (JFolder::exists($templatePath . $name))
						JFolder::delete($templatePath . $name);

				foreach (glob($templatePath . '/*.php') as $file)
					if (preg_match('/^jsn_/i', basename($file)))
						unlink($file);
			
				echo json_encode(array('update' => true, 'backup_file_name'=>'', 'message' => '', 'manual'=>false, 'redirect'=>false));
			}
			catch (Exception $ex)
			{
				echo json_encode(array('update' => true, 'backup_file_name'=>$backup_file, 'message' => $ex->getTraceAsString(), 'manual'=>false, 'redirect'=>false));
			}
		}
		else
		{
			echo json_encode(array('update'=>false, 'backup_file_name'=>'', 'message' => 'JSN_UPDATE_UNPACK_FAILED', 'manual'=>true, 'redirect'=>false));
		}
