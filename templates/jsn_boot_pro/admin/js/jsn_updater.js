...
var JSNUpdater = new Class({
	...
	dowloadTemplatePackage: function(template_name, url, style_id) {
		...
		var jsonRequest = new Request.JSON({
			...
			onSuccess: function(jsonObj)
			{
				if (typeof(jsonObj.redirect) != 'undefined' && jsonObj.redirect == true) {
					window.location = url + '?template=' + template_name + '&tmpl=jsn_updaternotification&template_style_id=' + style_id + '&type=auto';
					return;
				}
