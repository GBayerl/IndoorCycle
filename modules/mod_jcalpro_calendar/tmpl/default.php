...
<div id="jcalpro_calendar_<?php echo $id; ?>" class="jcalpro_calendar jcalpro_calendar<?php echo $moduleclass_sfx; ?>">
	...
	<div class="jcalpro_calendar_navbar">
		...
		<div class="jcalpro_calendar_month">
			<a href="<?php echo JCalProHelperFilter::escape(JCalProHelperUrl::events($dates->date->toRequest(), 'month')); ?>"><?php echo JCalProHelperFilter::escape($dates->date->format('F Y')); ?></a>
		</div>
	</div>
			...
			$cell_day = $row + $col; 
				if (array_key_exists($cell_day, $list)) :
					...
					if (empty($list[$cell_day]['events'])) :
						...
						$tip = '';
						if ($showtip) :
							$tip = JText::sprintf('MOD_JCALPRO_CALENDAR_EVENTS_FOR_X', $list[$cell_day]['user_datetime']->format('l, F d, Y')) . '::';
						...
						?><a class="jcalpro_calendar_tip jcalpro_calendar_tip_<?php echo $id; ?>" title="<?php echo JCalProHelperFilter::escape($tip); ?>" href="<?php echo JCalProHelperFilter::escape(JCalProHelperUrl::events($list[$cell_day]['user_datetime']->toRequest(), 'day', true, $urlparams)); ?>"><?php echo $cell_day; ?></a><?php
					endif;
					?></td><?php
				else :
