<?php
/**
 * @version		$Id: jcaldatetime.php 772 2012-04-17 19:21:09Z jeffchannell $
 * @package		JCalPro
...
class JFormFieldJCalDateTime extends JFormField
{
	...
	protected function getInput() {
		...
		if (!in_array('day', $excludes)) {
			$day = $this->_getRangedList($this->name . '[day]', $this->value['day'], 1, 31);
		}
		...
		if (!in_array('month', $excludes)) {
			...
			), $this->name . '[month]', $class, 'month_id', 'month_name', $this->value['month']);
		}
		// build the year select
		if (!in_array('year', $excludes)) {
			$year = $this->_getRangedList($this->name . '[year]', $this->value['year'], $today->year() - 5, $today->year() + 20);
		}
		...
		if (!in_array('hour', $excludes)) {
			...
			$hour = JHtml::_('select.genericlist', $hourlist, $this->name . '[hour]', $class . ' size="1"', 'hour_id', 'hour_name', $this->value['hour']);
		}
		// build the minutes select
		if (!in_array('minute', $excludes)) {
			$minute = $this->_getRangedList($this->name . '[minute]', $this->value['minute'], 0, 59, 'left');
		}
		// put it all together & return
		$hourminute = $hour . $minute;
		return $day . $month . $year . (empty($hourminute) ? '' : JText::_('COM_JCALPRO_DATETIME_AT')) . $hourminute;
	}
	
	private function _getRangedList($name, $value, $start = 0, $end = 0, $padding = false) {
		...
		return JHtml::_('select.genericlist', $list, $name, $class, 'range_id', 'range_name', $value);
