<?php
/**
 * @version		$Id: baseviewitem.php 799 2012-08-08 22:56:32Z jeffchannell $
 * @package		JCalPro
...
//		$this->script = $script;
	...
	public function addToolBar() {
		...
		$canDo = JCalPro::getActions();

		JToolBarHelper::title(JText::_(strtoupper(self::$option) . '_' . strtoupper($this->_name) . '_MANAGER_' . ($checkedOut ? 'VIEW' : ($isNew ? 'ADD' : 'EDIT'))), 'jcalpro-'.strtolower($this->_name));
	...
	public function setDocument() {
		...
		$document->setTitle(JText::_(strtoupper(self::$option).'_'.strtoupper($this->_name).'_'.($isNew ? 'CREATING' : 'EDITING')));
		if (isset(self::$script) && !empty(self::$script) && JFile::exists(JPATH_ROOT . self::$script)) {
			$document->addScript(JURI::root() . self::$script);
		}
		$submitbutton = "/administrator/components/" . strtolower(self::$option) . "/views/".strtolower($this->_name)."/submitbutton.js";
