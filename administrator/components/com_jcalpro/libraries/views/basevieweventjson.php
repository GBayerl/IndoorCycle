<?php
/**
 * @version		$Id: basevieweventjson.php 831 2012-10-30 18:16:47Z jeffchannell $
 * @package		JCalPro
...
class JCalProEventJsonView extends JCalProBaseView
{
		...
		switch ($input->get('layout', '', 'word')) {
			...
			case 'catcounts':
				...
				if (!empty($rcats)) {
					...
					foreach ($cats as $cat) {
						if (!in_array($cat->id, $rcats)) continue;
						$catObj = new stdClass();
						$catObj->id = $cat->id;
