<?php
/**
 * @version		$Id: basemodelevent.php 811 2012-10-09 15:51:00Z jeffchannell $
 * @package		JCalPro
...
JLoader::register('JCalProCustomFormModel', JPATH_ADMINISTRATOR.'/components/com_jcalpro/libraries/models/basemodelcustomform.php');
...
class JCalProEventModel extends JCalProCustomFormModel
{
	 ...
	 * @var		string
	 */
	public $_context = 'com_jcalpro.event';
	...
	/**
	 * The event to trigger before changing approval.
	 * 
	 * @var    string
	 */
	...
	public function __construct($config = array()) {
		...
		if (isset($config['event_change_approval'])) {
			$this->event_change_approval = $config['event_change_approval'];
		}
		elseif (empty($this->event_change_approval)) {
			$this->event_change_approval = 'onJCalChangeApproval';
		}
	}
	...
	public function getForm($data = array(), $loadData = true) {
		...
		if (!JCalPro::canModerateEvents($canonical)) $form->removeField('approved');
		...
		if (!JCalPro::canPublishEvents($canonical) && !$canCreatePrivate) $form->removeField('published');
		// only completely remove the registration if the user cannot create publis events
		if (!$canCreatePublic) $this->_removeRegistrationFields($form);
		/*
	...
	private function _getIcsFromRemote() {
		...
		// url cannot be empty!
		if (empty($url)) {
			$this->setError(JText::_('COM_JCALPRO_IMPORT_REMOTE_EMPTY_FILENAME'));
			return false;
		}
		...
		if (!preg_match('#^https?\:/{2}#', $url)) {
			$this->setError(JText::_('COM_JCALPRO_IMPORT_REMOTE_BAD_URL'));
			return false;
		}
		...
		if (!JFile::write($file, $data)) {
			$this->setError(JText::_('COM_JCALPRO_IMPORT_REMOTE_TMP_UNWRITABLE'));
			return false;
		}
		...
		foreach ($pks as $i => $pk) {
			if ($table->load($pk)) {
				if ($this->canModerate($table)) {
					...
					if (!$table->approve($pk, $state)) {
						$this->setError($table->getError());
						return false;
					}
					// Trigger the onContentChangeState event.
					$result = $dispatcher->trigger($this->event_change_approval, array($context, $pks, $state));
