<?php
/**
 * @version		$Id: mail.php 834 2012-11-13 17:58:50Z jeffchannell $
 * @package		JCalPro
...
abstract class JCalProHelperMail
{
	...
	static public function send($context, $event, $user, $subject = false, $body = false) {
		...
		if (!$user) {
			...
			if (!self::canSendTo($user->email)) return;
		}
		$lang = '';
		if (method_exists($user, 'getParam')) {
			...
			$lang = JFactory::getUser()->getParam('language', JFactory::getUser()->getParam('admin_language'));
		}
		// do we have a language? if not use site default
		if (empty($lang)) $lang = JFactory::getLanguage()->getTag();
		...
		if (!empty($lang)) $query->where($db->quoteName('language') . ' IN (' . $db->quote('*') . ', ' . $db->quote($lang) . ')');
		
		$db->setQuery($query);
		
		try {
			$email = $db->loadObject();
			if (empty($email)) throw new Exception('No email!');
		...
		$email->body = self::replaceTags((!empty($body) ? $body : $email->body), $event, $user);
		$email->subject = self::replaceTags((!empty($subject) ? $subject : $email->subject), $event, $user);
		
		return self::mail($user->email, $email->subject, $email->body);
	}
	...
	static public function getModerators($catid = null) {
		...
		foreach ($groups as $group) {
			...
			if (JAccess::checkGroup($group, 'core.moderate', $asset) || JAccess::checkGroup($group, 'core.admin')) {
				...
				if (!empty($gusers)) $users = array_merge($users, $gusers);
			}
		}
	...
	static public function canSendTo($email) {
		...
		// get the value
		try {
			$emails[$email] = ($db->loadResult() == $email);
			return $emails[$email];
		}
		catch (Exception $e) {
	 ...
	 * @return array
	 */
	public static function getEmailTags($filter = null, $keys = true) {
		static $tags;
		if (!is_array($tags)) {
			$tags = array(
				// site variables
			...
			if (!empty($custom)) {
				...
				foreach ($custom as $c) {
					...
					if (JText::_($lang) == $lang) $lang = 'COM_JCALPRO_EMAIL_TAG_USER_DEFINED';
					$tags[$c] = $lang;
				}
			}
	...
	public static function replaceRegistrationTags($string, $event) {
		...
		if (!property_exists($event, 'registration_data') || !property_exists($event->registration_data, 'current_entry')) return $string;
	...
	public static function replaceSiteTags($string) {
		...
		if (is_null($site)) {
			$site = new stdClass;
			$site->url = JUri::root();
			$site->fromname = JFactory::getApplication()->getCfg('fromname');
			$site->mailfrom = JFactory::getApplication()->getCfg('mailfrom');
			$site->name = JFactory::getApplication()->getCfg('sitename');
		}
	...
	public static function replaceTags($string, $event, $user) {
		$string = self::replaceEventTags($string, $event);
		$string = self::replaceRegistrationTags($string, $event);
		$string = self::replaceUserTags($string, $user);
		$string = self::replaceSiteTags($string);
		$string = self::replaceCustomTags($string, $event, $user);
		$string = self::deleteAllTags($string);
		return $string;
	}
	
	private static function _replaceTags($string, $namespace, $object, $extra = false) {
		$tags = self::getEmailTags($namespace);
		...
		if (!empty($tags)) foreach ($tags as $tag) {
			if (false === stripos($string, $tag)) continue;
			$parts = explode('.', $tag);
