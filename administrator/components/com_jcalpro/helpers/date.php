<?php
/**
 * @version		$Id: date.php 800 2012-09-06 02:33:01Z jeffchannell $
 * @package		JCalPro
...
abstract class JCalProHelperDate
{
	...
	/**
	 * returns an array with day, month, year, hour, minute keys
	 * this array is is USER TIME, not UTC
	 * 
	...
	public static function getToday() {
		...
		if (!isset($today)) {
			...
			$today = JCalDate::createFromMySQLFormat($date->format('Y-m-d 00:00:00', true), $tz);
		}
		return clone $today;
	}
	...
	public static function getNthDayOfMonth($nth, $weekday, $month, $year) {
		...
		if (!in_array($nth, range(1, 5))) return false;
		// $day should be an integer from 0-6
		if (!in_array($weekday, range(0, 6))) return false;
		...
		$datetime = self::getDateTimeFromParts(0, 0, 0, $month, 1, $year, 'UTC');
		// if we're looking for the last day, move our DateTime to the end
		if ($reverse) {
			$datetime->toMonthEnd()->toDayStart();
			//$days = self::getDaysInMonth($datetime);
			//$datetime = self::getDateTimeFromParts(0, 0, 0, $month, $days, $year, 'UTC');
		}
	...
	public static function format($ts, $format = null, $local = false) {
		$jcDate = new JDate($ts);
		if (is_null($format)) $format = JCalDate::JCL_FORMAT_MYSQL;
