<?php
/**
 * @version		$Id: form.php 799 2012-08-08 22:56:32Z jeffchannell $
 * @package		JCalPro
...
class JCalProTableForm extends JTable
{
	public $id;
	public $type;
	public $title;
	...
	public function store($updateNulls = false) {
		...
		if ($store) {
			...
			$this->_db->setQuery('DELETE FROM #__jcalpro_form_fields WHERE form_id=' . intval($this->id));
			$this->_db->query();
			...
			if (!empty($formfields)) {
				...
				for ($i = 0; $i < count($formfields); $i++) {
					$formfields[$i] = "(" . intval($this->id) . ", " . $formfields[$i] . ", $i)";
				}
				// create a new db query & inject the new records
				$this->_db->setQuery('INSERT IGNORE INTO #__jcalpro_form_fields (form_id, field_id, ordering) VALUES ' . implode(', ', $formfields));
				$this->_db->query();
			}
		}
