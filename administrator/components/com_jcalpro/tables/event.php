<?php
/**
 * @version		$Id: event.php 835 2012-11-26 23:39:14Z jeffchannell $
 * @package		JCalPro
...
class JCalProTableEvent extends JTable
{
	...
	private $_cat_ids; // for categories
	...
	protected function _getAssetParentId($table = null, $id = null) {
		...
		$asset->loadByName('com_jcalpro' . ($catid ? '.category.' . $catid : ''));
		return $asset->id;
	}
	...
	public function loadByCommonId($common_id, $reset = true) {
		...
		$db->setQuery((string) $db->getQuery(true)
			...
			->where('common_event_id = ' . $db->Quote($common_id))
			->where('rec_id = 0')
		);
		$id = $db->loadResult();
		if (empty($id)) $id = null;
			...
			switch ((int) $this->recur_type) {
				...
				case JCalPro::RECUR_TYPE_WEEKLY :
					...
					if (0 == array_sum($this->_parsed_rec_weekly)) {
						$this->setError(JText::_('COM_JCALPRO_REC_WEEKLY_NO_DAYS_CHECKED'));
						return false;
					}
					// check that this event falls on one of our recurring days
					// but only after cloning the DateTime and putting it back to the configured timezone
					$wdatetime = clone $this->_basedatetime;
					$wdatetime->toTimezone($this->timezone);
					if (0 == $this->_parsed_rec_weekly[(int) $wdatetime->weekday()]) {
						$this->setError(JText::_('COM_JCALPRO_REC_INVALID_START_DAY'));
						return false;
					}
	...
	public function store($updateNulls = false) {
		...
		// for now just store these via getDate
		...
		if (property_exists($this, 'canonical')) unset($this->canonical);
		if (property_exists($this, 'cat')) unset($this->cat);
		...
		if (!$this->id) {
			...
			// public events need to be moderated (and not published automatically)
				...
				if (!$canModerate)  $this->approved  = (bool) (int) $canAutoApprove;
				if (!$canEditState) $this->published = (bool) (int) $canAutoPublish;
			}
			// set created information
			if (!intval($this->created))  $this->created    = $jdate->toSql();
		...
		// existing event
			...
			if ($this->rec_id) $this->detached_from_rec = 1;
		...
		switch ($this->duration_type) {
			...
			// NOTE: if the interval ends up being blank, we don't break here!!!
			case '1':
				...
				$this->end_date = $enddatetime->addDay($this->end_days)->addHour($this->end_hours)->addMin($this->end_minutes)->toSql();
				break;
		...
		if ($store) {
			...
			if (0 == $this->rec_id && $this->recur_type) {
				...
				if (2 == $this->recur_end_type) {
					...
					$enddatetime = JCalProHelperDate::getDateTimeFromParts(0, 0, 0, $endparts->month, $endparts->day, $endparts->year, $this->timezone);
				}
				else {
					$repeatedcount = 0;
					...
					case JCalPro::RECUR_TYPE_YEARLY :
						...
						while (true) {
							...
							// otherwise we need to find the nth day, so we use the date helper to grab that
								...
								if ($nth) $children[] = $this->_cloneAndStoreRecurrence((int) $nth->day(), (int) $nth->month(), (int) $nth->year());
							}
							
							// increment counters
							$recurbasedatetime->addX($period, $interval);
							if (1 == $this->recur_end_type) $repeatedcount++;
						}
	...
	public function bind($array, $ignore = '') {
		...
		foreach (self::$_date_array_parts as $key => $prefix) {
			if (array_key_exists($key, $array) && is_array($array[$key])) {
				...
				foreach (self::$_time_vars as $var) {
					...
					if (!array_key_exists($var, $array[$key])) continue;
		...
		foreach (array('sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday') as $day) {
			$key = "rec_weekly_on_{$day}";
			if (!array_key_exists($key, $array)) $array[$key] = 0;
		}
		
		// TODO: why is this here?
		if (array_key_exists('catid', $array)) unset($array['catid']);
	...
	public function approve($pks = null, $state = 1, $userId = 0) {
		...
		if (!$this->_db->query()) {
			$e = new JException(JText::sprintf('COM_JCALPRO_ERROR_APPROVED_FAILED', get_class($this), $this->_db->getErrorMsg()));
			$this->setError($e);
