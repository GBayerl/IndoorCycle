...
class JCalProViewEvents extends JCalProListView
{
	function display($tpl = null, $echo = true) {
		...
		if (!empty($items)) {
			foreach ($items as $item) {
				if ($item->rec_id && 0 == $item->detached_from_rec) {
					if (!array_key_exists($item->rec_id, $children)) $children[$item->rec_id] = array();
		...
		$script = array();
		$script[] = 'window.jclChildrenEvents = {data:[],list:[]};';
		if (!empty($children)) {
			$kids = array();
		...
		// language filter
		$opts = array(JHtml::_('select.option', '', JText::_('JOPTION_SELECT_LANGUAGE')));
