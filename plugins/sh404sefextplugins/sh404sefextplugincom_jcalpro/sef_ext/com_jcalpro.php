<?php
/**
 * @version		$Id: com_jcalpro.php 790 2012-05-28 19:36:06Z jeffchannell $
 * @package		JCalPro
...
$shJCalPro = empty($shJCalPro) ? getMenuTitle($option, $view, $Itemid, null, $shLangName) : $shJCalPro;
$shJCalPro = (empty($shJCalPro) || $shJCalPro == '/') ? 'JCalPro' : $shJCalPro;
...
if ('modal' == $layout || 'component' == $tmpl || 'json' == $format || !empty($task)) $dosef = false;
		...
		if (is_object($category) && method_exists($category, 'getPath')) {
			...
			if (!empty($path)) {
				...
				foreach ($parts as $p) {
					...
					if ($pCategoryId) $part .= "$cid-";
					if ($pCategoryAlias) $part .= $calias;
					if (!empty($part)) {
						$title[] = $part;
						$title[] = '/';
					}
		...
		switch ($layout) {
			...
			default:
				...
				if ($data = $db->loadObject()) {
					...
					// add the title/id
					$etitle = '';
					if ($pEventId) $etitle .= "$id-";
					if ($pEventAlias) $etitle .= $data->alias;
					if (!empty($etitle)) {
						$title[] = $etitle;
					}
					shRemoveFromGETVarsList('view');
