<?php
/**
 * @version		$Id: default_location_events.php 822 2012-10-22 23:06:00Z jeffchannell $
 * @package		JCalPro
		...
		<div class="jcl_event_body jcl_nooverflow" style="border-left-color: <?php echo @$event->color; ?>;">
			<h3>
				<?php
					...
					$time = $event->user_minidisplay . ' ' . $event->user_timedisplay;
					if (property_exists($event, 'user_end_minidisplay')) :
						$time = $event->user_minidisplay . ' ';
						if ($event->user_minidisplay != $event->user_end_minidisplay) :
							$time .= $event->user_start_timedisplay . ' - ' . $event->user_end_minidisplay . ' ' . $event->user_end_timedisplay;
						else :
