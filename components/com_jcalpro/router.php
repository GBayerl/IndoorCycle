<?php
/**
 * @version		$Id: router.php 794 2012-06-06 23:24:12Z jeffchannell $
 * @package		JCalPro
...
function JcalproBuildRoute(&$query) {
	...
	if ('category' == $view) {
		if (array_key_exists('id', $query)) {
			...
			$categories = JCategories::getInstance('JCalPro');
			$category = $categories->get($id);
			if (is_object($category) && method_exists($category, 'getPath')) {
				$segments = array_merge($segments, $category->getPath());
			}
			unset($query['id']);
		}
	}
...
function JcalproParseRoute($segments) {
	...
	switch ($segments[0]) {
		...
		case 'admin':
			$vars['view']   = 'events';
			$vars['layout'] = $segments['events' == $segments[0] ? 1 : 0];
			return $vars;
		...
		default:
			...
			if (1 == $count) {
				if (false === strpos(':', $segments[0])) $category = $segments[0];
				else list($category, $tmp) = explode(':', $segments[0], 2);
				$vars['view'] = 'category';
				$vars['id'] = $category;
				return $vars;
			}
