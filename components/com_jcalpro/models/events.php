<?php
/**
 * @version		$Id: events.php 800 2012-09-06 02:33:01Z jeffchannell $
 * @package		JCalPro
...
class JCalProModelEvents extends JCalProListEventsModel
{
	...
	protected function getListQuery() {
		...
		if (empty($start_date)) $start_date = JCalProHelperDate::JCL_DATE_MIN;
		if (empty($end_date))   $end_date   = JCalProHelperDate::JCL_DATE_MAX;
		...
		if (intval($this->getState('filter.date_range')) || 'admin' == $layout || !empty($search)) {
	...
	public function getLinkData() {
		...
		switch ($layout) {
			...
			case 'flat':
				...
				$data['current']      = $dates->date->format(JText::_('COM_JCALPRO_DATE_FORMAT_MONTH_YEAR'));
				$data['next']['text'] = $dates->next_month->format(JText::_('COM_JCALPRO_DATE_FORMAT_MONTH_YEAR'));
				if (!(!$archive && $dates->date_month == $dates->today_month && $dates->date_year == $dates->today_year)) {
					$data['prev']['text'] = $dates->prev_month->format(JText::_('COM_JCALPRO_DATE_FORMAT_MONTH_YEAR'));
				}
				break;
			case 'week':
				...
				break;
			case 'day':
				$data['current']      = $dates->date->format(JText::_('COM_JCALPRO_DATE_FORMAT_FULL_DATE'));
