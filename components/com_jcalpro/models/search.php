<?php
/**
 * @version		$Id: search.php 838 2012-11-29 19:44:22Z jeffchannell $
 * @package		JCalPro
...
class JCalProModelSearch extends JCalProListModel
{
	 ...
	 * @var		string
	 */
	public $_context = 'com_jcalpro.search';
	...
	function __construct($config = array()) {
		...
		$model->populateState();
		//$model->setCategoryFilters(array());
	...
	protected function getListQuery() {
		...
		if (empty($search)) $query->where('1=2');
	...
	public function getItems() {
		...
		if (!empty($items)) foreach ($items as &$item) {
			...
			if (empty($item->categories->canonical)) continue;
			$final[] = $item;
		}
